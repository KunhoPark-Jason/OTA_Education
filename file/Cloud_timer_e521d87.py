...
DEFAULT_PRIVKEY_PATH = os.path.join(certs_ecdsa_dir, "ecdsa521_private.pem")

# PQC 키 경로: /home/sea/OTA/OTA_Education/certs/pqc
certs_pqc_dir = os.path.join(certs_root_dir, "pqc")
PQC_PRIVKEY_PATH = os.path.join(certs_pqc_dir, "pqc_private_d87.key")
...
def make_message_and_signatures(file_path: str, private_key):
    ...
    파일을 읽어서:
      ...
      - pqc_sig_b64   : Falcon 서명(base64)
    를 생성해 반환한다.
    """

    # 1) 파일 open/read (바이트 로딩)
    t0 = _now_ms()
    with open(file_path, "rb") as f:
        file_bytes = f.read()
    _perf_ms("CLOUD.1.file_read", t0)

    # 1) 파일 데이터 base64 인코딩 (MQTT payload)
    t0 = _now_ms()
    message = base64.b64encode(file_bytes)
    _perf_ms("CLOUD.1.file_base64", t0)

    # 2) ECDSA 서명 생성
    t0 = _now_ms()
    ecdsa_sig = sign_data_ecdsa(private_key, file_bytes)
    _perf_ms("CLOUD.2.ecdsa_sign", t0)

    # 3) PQC 서명 생성
    t0 = _now_ms()
    pqc_sig = pqc_sign(file_bytes)
    _perf_ms("CLOUD.3.pqc_sign", t0)

    # 4) 각 서명을 base64 인코딩
    t0 = _now_ms()
    ecdsa_sig_b64 = base64.b64encode(ecdsa_sig)
    pqc_sig_b64 = base64.b64encode(pqc_sig)
    _perf_ms("CLOUD.4.sigs_base64", t0)
...
def send_file_to_broker(broker_ip, username, password,
    ...
    try:
        ...
        while True:
            # 1) 디렉터리 스캔(os.listdir)
            t0 = _now_ms()
            files = os.listdir(publish_dir)

            if not files:
                time.sleep(1)   # 50ms 폴링 (원하는 값으로 조절)
                continue

            _perf_ms("CLOUD.1.listdir", t0)
            ...
            for file in files:
                ...
                try:
                    ...
                    client.loop_stop()
                    _perf_ms("CLOUD.5.publish_4topics", t0)
