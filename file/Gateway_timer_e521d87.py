...
def _perf_ms(tag: str, t0_ms: float):
    dt = _now_ms() - t0_ms
    print(f"[PERF] {tag}: {dt:.3f} ms")
...
PUBLIC_KEY_PATH = os.path.join(certs_ecdsa_dir, "ecdsa521_public.pem")

# PQC 키 경로: /home/sea/OTA/OTA_Education/certs/pqc
certs_pqc_dir = os.path.join(certs_root_dir, "pqc")
PQC_PUBKEY_PATH = os.path.join(certs_pqc_dir, "pqc_public_d87.key")
...
PQC_ALG_NAME = b"ML-DSA-87"
PQC_MAX_SIG_LEN = 8192
...
def verify_vg():
    ...
    if not verify_ecdsa_signature(public_key, vg_data, vg_sig_ecdsa):
        _perf_ms("GW.11.vg_ecdsa_verify", t0)
    ...
    if not pqc_verify(vg_data, vg_sig_pqc):
        _perf_ms("GW.12.vg_pqc_verify", t0)
        print("[FAIL] VG PQC verify")
        return False
    _perf_ms("GW.12.vg_pqc_verify", t0)

    # (13) valid_until 만료 체크
    t0 = _now_ms()
    if is_vg_expired(vg_data):
        _perf_ms("GW.13.vg_valid_until_check", t0)
...
def generate_tokens():
    ...
    _perf_ms("GW.14.sha256_ota", t0)

    t0 = _now_ms()
    vg_hash  = hashlib.sha256(vg_data).digest()
    _perf_ms("GW.14.sha256_vg", t0)

    t0 = _now_ms()
    master_key = _load_master_key()
    _perf_ms("GW.14.load_master_key", t0)
    ...
    for entry in vg.get("allowed_transitions", []):
        ...
        token32 = _make_token(master_key, ecu_id, ota_hash, vg_hash, nonce16)
        _perf_ms(f"GW.14.token_hmac.{ecu_id}", t0)
        ...
        try:
            for ecu_id in ecu_list:
                ...
                if sig_bytes is not None:
                    ...
                    _perf_ms(f"GW.30.sig_stream.{ecu_id}", t0)
                else:
...
def save_file_if_complete():
    ...
    try:
        ...
        with open(save_path, "wb") as f:
            f.write(file_data)
        _perf_ms("GW.7.temp_write_file", t0)
    ...
    _perf_ms("GW.9.file_ecdsa_verify", t0)

    try:
        t0 = _now_ms()
        ok_pqc = pqc_verify(file_data, file_signature_pqc)
        _perf_ms("GW.10.file_pqc_verify", t0)
    except Exception as e:
...
def on_message(client, userdata, msg):
    ...
    if topic == name_topic:
        ...
        try:
            ...
            _perf_ms("GW.7.decode_file_base64", t0)
        ...
        try:
            ...
            _perf_ms("GW.7.decode_sig_ecdsa_base64", t0)
        ...
        try:
            ...
            _perf_ms("GW.7.decode_sig_pqc_base64", t0)
        ...
        _perf_ms("GW.7.decode_vg_base64", t0)
...
def main():
    ...
    _perf_ms("GW.6.subscribe_all", t0)
