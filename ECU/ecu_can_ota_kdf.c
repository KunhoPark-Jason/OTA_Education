...
// START~END 프레이밍으로 들어오는 스트림을 모아서 버퍼로 반환(8바이트 단위)
...
static void derive_ecu_key(const uint8_t master32[32], const char *ecu_id, uint8_t out32[32]) {
    ...
    HMAC(EVP_sha256(), master32, 32, (const unsigned char*)msg, (int)strlen(msg), out32, &outlen);
}
...
int main(int argc, char **argv) {
    if (argc < 7) {
        fprintf(stderr, "usage: %s <can_if> <ECU_ID> <ECU_ADDR_HEX> <serial_txt> <out_bin> <master_key_bin>\n", argv[0]);
        return 1;
    }
    ...
    while (1) {
        ...
        // 2) META stream: nonce16 + be32(pq_len) + pq_bytes + vg_hash32 + ota_hash32
        if (id == META_ID) {
            ...
            if (recv_stream(s, META_ID, &buf, &n) == 0) {
                if (n >= 16 + 4 + 32 + 32) {
                    ...
                    if (n >= need) {
                        // pq_bytes는 필요하면 활용 가능(여기선 저장만 생략)
        ...
        // 4) OTA stream: raw bytes -> out_bin 저장 후 sha256 검증
        if (id == OTA_ID) {
            uint8_t *buf = NULL; size_t n = 0;
